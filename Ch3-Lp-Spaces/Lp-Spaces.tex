...
\begin{exercise}\textbf{Exercise 3.7.}
    ...
    Some conditions which imply inclusion can be found in the section of useful properties.
\end{exercise}
...
Let $0 < r < p < s < +\infty$ with $p-r=s-p$ which is the same as $2p=r+s$ and let $f$ be a masurable function, using Holder's inequality we get,  
\[
...
\begin{observation}\textbf{Observation 3.2}
    Let $1 \le r < p < \infty$ and let $f$ be a measurable function, using Holder's inequality we get,
    \[
        \|f\|_r = \left( \|f^r\|_1  \right)^{\frac{1}{r}} \le \left( \|1\|_q \|f^r\|_ \frac{p}{r} \right)^\frac{1}{r} = \mu \left( X \right) ^\frac{1}{rq} \|f\|_p   
    \]
    with $q = \left( 1 - \frac{r}{p} \right)^{-1}$

    Observe that if $\mu \left( X \right) < \infty$ we get that $L^p \left( \mu \right) \subset L^r \left( \mu \right) $.
\end{observation}
